...
Q.Circuit.Editor = function (circuit, targetEl) {
  ...
  this.name =
    typeof circuit.name === 'string' ? circuit.name : 'Q Editor ' + this.index
  ...
  //  into a usable domId.
    ...
    let domIdBase = this.name.replace(/^[^a-z]+|[^\w:.-]+/gi, '-'),
      domId = domIdBase,
      domIdAttempt = 1
  ...
  for (let i = 0; i < circuit.bandwidth; i++) {
    ...
    rowEl.style.gridColumnEnd =
      Q.Circuit.Editor.momentIndexToGridColumn(circuit.timewidth) + 1
  ...
  for (let i = 0; i < circuit.timewidth; i++) {
    ...
    columnEl.style.gridRowEnd =
      Q.Circuit.Editor.registerIndexToGridRow(circuit.bandwidth) + 1
  ...
  for (let i = 0; i < circuit.bandwidth; i++) {
    ...
    registersymbolEl.classList.add(
      'Q-circuit-header',
      'Q-circuit-register-label'
    )
    registersymbolEl.setAttribute(
      'title',
      'Register ' + registerIndex + ' of ' + circuit.bandwidth
    )
    registersymbolEl.setAttribute('register-index', registerIndex)
    registersymbolEl.style.gridRowStart =
      Q.Circuit.Editor.registerIndexToGridRow(registerIndex)
    registersymbolEl.innerText = 'Nr. ' + registerIndex
  }
  ...
  addRegisterEl.style.gridRowStart = Q.Circuit.Editor.registerIndexToGridRow(
    circuit.bandwidth + 1
  )
  ...
  for (let i = 0; i < circuit.timewidth; i++) {
    ...
    momentsymbolEl.setAttribute(
      'title',
      'Moment ' + momentIndex + ' of ' + circuit.timewidth
    )
    momentsymbolEl.setAttribute('moment-index', momentIndex)
    momentsymbolEl.style.gridColumnStart =
  ...
  addMomentEl.style.gridColumnStart = Q.Circuit.Editor.momentIndexToGridColumn(
    circuit.timewidth + 1
  )
  ...
  circuit.qubits.forEach(function (qubit, i) {
    ...
    inputEl.style.gridRowStart =
      Q.Circuit.Editor.registerIndexToGridRow(rowIndex)
...
Object.assign(Q.Circuit.Editor, {
  ...
  dragEl: null,
  gridColumnToMomentIndex: function (gridColumn) {
    return +gridColumn - 2
  },
  momentIndexToGridColumn: function (momentIndex) {
    return momentIndex + 2
  },
  gridRowToRegisterIndex: function (gridRow) {
    return +gridRow - 1
  },
  registerIndexToGridRow: function (registerIndex) {
  ...
  createPalette: function (targetEl, gates = 'HXYZPT*') {
    ...
    gates.split('').forEach(function (symbol) {
      ...
      ;['before', 'after'].forEach(function (layer) {
        tileEl.style.setProperty(
          '--Q-' + layer + '-rotation',
          randomRangeAndSign(0.5, 4) + 'deg'
        )
        tileEl.style.setProperty(
          '--Q-' + layer + '-x',
          randomRangeAndSign(1, 4) + 'px'
        )
        tileEl.style.setProperty(
          '--Q-' + layer + '-y',
          randomRangeAndSign(1, 3) + 'px'
        )
      })
    })
...
Q.Circuit.Editor.set = function (circuitEl, operation) {
  const backgroundEl = circuitEl.querySelector('.Q-circuit-board-background'),
  ...
  operation.registerIndices.forEach(function (registerIndex, i) {
    ...
    operationEl.classList.add(
      'Q-circuit-operation',
      'Q-circuit-operation-' + operation.gate.nameCss
    )
    ...
    operationEl.style.gridColumnStart =
      Q.Circuit.Editor.momentIndexToGridColumn(operation.momentIndex)
    operationEl.style.gridRowStart =
      Q.Circuit.Editor.registerIndexToGridRow(registerIndex)
    ...
    if (operation.gate.symbol !== Q.Gate.CURSOR.symbol)
      tileEl.innerText = operation.gate.symbol
    ...
    if (operation.registerIndices.length > 1) {
      ...
      operationEl.setAttribute(
        'sibling-indices',
        operation.registerIndices.filter(function (siblingRegisterIndex) {
          return registerIndex !== siblingRegisterIndex
        })
      )
      operation.registerIndices.forEach(function (registerIndex, i) {
        if (i < operation.registerIndices.length - 1) {
          const siblingRegisterIndex = operation.registerIndices[i + 1],
          ...
          containerEl.style.gridRowStart =
            Q.Circuit.Editor.registerIndexToGridRow(start)
          containerEl.style.gridRowEnd =
            Q.Circuit.Editor.registerIndexToGridRow(end + 1)
          containerEl.style.gridColumn =
            Q.Circuit.Editor.momentIndexToGridColumn(operation.momentIndex)

          containerEl.appendChild(linkEl)
          linkEl.classList.add('Q-circuit-operation-link')
          if (registerDelta > 1)
            linkEl.classList.add('Q-circuit-operation-link-curved')
        }
      })
...
Q.Circuit.Editor.isValidControlCandidate = function (circuitEl) {
  const selectedOperations = Array.from(
    circuitEl.querySelectorAll('.Q-circuit-cell-selected')
  )
  ...
  const allSiblingsPresent = selectedOperations.reduce(function (
    status,
    operationEl
  ) {
    ...
    const registerIndicesLength = registerIndicesString
        .split(',')
      ...
      allSiblingsLength = selectedOperations.reduce(function (
        siblings,
        operationEl
      ) {
        if (
          operationEl.getAttribute('register-indices') === registerIndicesString
        ) {
  ...
  const { totalControlled, totalNotControlled } = selectedOperations.reduce(
    function (stats, operationEl) {
  ...
  if (
    gates[Q.Gate.CURSOR.symbol] === undefined &&
...
Q.Circuit.Editor.createControl = function (circuitEl) {
  ...
  const circuit = circuitEl.circuit,
    ...
    control =
      existingControlEl ||
      selectedOperations.find(function (el) {
...
Q.Circuit.Editor.createSwap = function (circuitEl) {
  ...
  ;(registerIndices = selectedOperations.reduce(function (
    registerIndices,
    operationEl
  ) {
  ...
  circuit.set$(Q.Gate.SWAP, momentIndex, registerIndices)
...
Q.Circuit.Editor.onCircuitChanged = function (circuitEl) {
  ...
  window.dispatchEvent(
    new CustomEvent('Q gui altered circuit', { detail: { circuit: circuit } })
  )
...
Q.Circuit.Editor.unhighlightAll = function (circuitEl) {
  Array.from(
    circuitEl.querySelectorAll(
      '.Q-circuit-board-background > div,' + '.Q-circuit-board-foreground > div'
    )
  ).forEach(function (el) {
...
Q.Circuit.Editor.onPointerMove = function (event) {
  ...
  if (Q.Circuit.Editor.dragEl !== null) {
    ...
    Q.Circuit.Editor.dragEl.style.left =
      x + window.pageXOffset + Q.Circuit.Editor.dragEl.offsetX + 'px'
    Q.Circuit.Editor.dragEl.style.top =
      y + window.pageYOffset + Q.Circuit.Editor.dragEl.offsetY + 'px'

    if (!boardContainerEl && Q.Circuit.Editor.dragEl.circuitEl)
      Q.Circuit.Editor.dragEl.classList.add('Q-circuit-clipboard-danger')
  ...
  Array.from(
    boardContainerEl.querySelectorAll(`
  ...
  const cellEl = foundEls.find(function (el) {
    ...
    highlightByQuery = function (query) {
      Array.from(boardContainerEl.querySelectorAll(query)).forEach(function (
        el
      ) {
  ...
  if (cellEl) {
    const momentIndex = cellEl.getAttribute('moment-index'),
  ...
  const boardElBounds = boardContainerEl.getBoundingClientRect(),
  ...
  if (
    momentIndex > circuitEl.circuit.timewidth ||
    registerIndex > circuitEl.circuit.bandwidth
  )
    return
...
Q.Circuit.Editor.onPointerPress = function (event) {
  ...
  if (circuitEl) {
    ...
    if (circuitIsLocked) {
      Q.warn(
        `User attempted to interact with a circuit editor but it was locked.`
      )
      return
    }
    ...
    const selectallEl = targetEl.closest('.Q-circuit-selectall'),
    ...
    if (inputEl) {
      ...
      circuit.qubits[registerIndex - 1] = circuit.qubits[
        registerIndex - 1
    ...
    function toggleSelection(query) {
      const operations = Array.from(circuitEl.querySelectorAll(query)),
    ...
    selectedOperations.forEach(function (el) {
      ...
      const momentIndex = +el.getAttribute('moment-index'),
    ...
    selectedOperations.forEach(function (el) {
      const columnIndexForClipboard =
          1 + el.origin.columnIndex - columnIndexMin,
      ...
      if (
        el.origin.columnIndex === columnIndex &&
    ...
    const boardEl = circuitEl.querySelector('.Q-circuit-board-foreground'),
    ...
    const bounds = operationEl.getBoundingClientRect(),
...
Q.Circuit.Editor.onPointerRelease = function (event) {
  ...
  const draggedOperations = Array.from(Q.Circuit.Editor.dragEl.children),
    draggedMomentDelta =
      droppedAtMomentIndex - Q.Circuit.Editor.dragEl.momentIndex,
    draggedRegisterDelta =
      droppedAtRegisterIndex - Q.Circuit.Editor.dragEl.registerIndex,
    setCommands = []
  ...
  draggedOperations.forEach(function (childEl, i) {
    let momentIndexTarget = droppedAtMomentIndex,
      registerIndexTarget = droppedAtRegisterIndex

    if (Q.Circuit.Editor.dragEl.circuitEl) {
      momentIndexTarget +=
        childEl.origin.momentIndex - Q.Circuit.Editor.dragEl.momentIndex
      registerIndexTarget +=
        childEl.origin.registerIndex - Q.Circuit.Editor.dragEl.registerIndex
    }
    ...
    if (registerIndicesString) {
      ...
      const registerIndices = registerIndicesString
          .split(',')
          .map(function (str) {
            return +str
          }),
        ...
        foundComponents = Array.from(
          ...
          const aRegisterIndicesIndex = +a.getAttribute(
              'register-indices-index'
            ),
            bRegisterIndicesIndex = +b.getAttribute('register-indices-index')

          return aRegisterIndicesIndex - bRegisterIndicesIndex
        }),
        allComponents = Array.from(
          Q.Circuit.Editor.dragEl.circuitEl.querySelectorAll(
            `[moment-index="${childEl.origin.momentIndex}"]` +
              `[register-indices="${registerIndicesString}"]`
          )
        ),
      ...
      //  that might be ok!
        ...
        const registerMap = registerIndices.reduce(function (
          registerMap,
          registerIndex,
          r
        ) {
        ...
        foundComponents.forEach(function (component) {
          const componentRegisterIndex =
            +component.getAttribute('register-index')
        ...
        foundComponents.forEach(function (component) {
          const componentRegisterIndex =
              +component.getAttribute('register-index'),
            registerGrabDelta =
              componentRegisterIndex - Q.Circuit.Editor.dragEl.registerIndex

          //  Now put it where it wants to go,
          //  possibly overwriting a sibling component!

          registerMap[componentRegisterIndex + draggedRegisterDelta] =
      ...
      while (j < draggedOperations.length) {
        ...
        if (
          possibleSibling.getAttribute('gate-symbol') === gatesymbol &&
          possibleSibling.getAttribute('register-indices') ===
            registerIndicesString
        ) {
  ...
  if (
    Q.Circuit.Editor.dragEl.circuitEl &&
    Q.Circuit.Editor.dragEl.circuitEl !== circuitEl
  ) {
    const originCircuitEl = Q.Circuit.Editor.dragEl.circuitEl
    Q.Circuit.Editor.onSelectionChanged(originCircuitEl)
